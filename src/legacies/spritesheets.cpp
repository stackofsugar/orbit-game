...
public:
    ...
    void render(int x, int y, SDL_Rect *clip) {
        ...
        if (clip != NULL) {
            renderTarget.w = clip->w;
            renderTarget.h = clip->h;
        }

        SDL_RenderCopy(g_renderer, m_texture, clip, &renderTarget);
    }
...
bool loadMedia() {
    if (!g_spritesheet.loadFromFile("res2//sprites.png")) {
        return false;
    }

    SDL_Surface *icon = IMG_Load("res2//sprites.png");
    if (!icon) {
        return false;
    }
    ...
    g_spriteClips[0] = { 0,0,100,100 };
    g_spriteClips[1] = { 100,0,100,100 };
    g_spriteClips[2] = { 0,100,100,100 };
    g_spriteClips[3] = { 100,100,100,100 };
    return true;
}
...
int main(int argc, char **argv) {
    ...
    while (running) {
        ...
        if (screenUpdate) {
            screenUpdate = false;
            SDL_RenderClear(g_renderer);
            g_spritesheet.setColor(r, g, b);
            g_spritesheet.render(0, 0, &g_spriteClips[0]);
            g_spritesheet.render(SCREEN_W - g_spriteClips[1].w, 0, &g_spriteClips[1]);
            g_spritesheet.render(0, SCREEN_H - g_spriteClips[2].h, &g_spriteClips[2]);
            g_spritesheet.render(SCREEN_W - g_spriteClips[3].w,
                                 SCREEN_H - g_spriteClips[3].h, &g_spriteClips[3]);
            SDL_RenderPresent(g_renderer);
        }
